class InfiniteMarquee {
    constructor(e = {}) {
        (this.element = "string" == typeof e.element ? [...document.querySelectorAll(e.element)] : e.element),
            ...
            (this.gap = { vertical: (e.gap && e.gap.vertical) || "5px", horizontal: (e.gap && e.gap.horizontal) || "0px" }),
            ...
            (this.on = { beforeInit: (e.on && e.on.beforeInit) || null, afterInit: (e.on && e.on.afterInit) || null, pauseAnimation: (e.on && e.on.pauseAnimation) || null, resumeAnimation: (e.on && e.on.resumeAnimation) || null }),
            ...
            "undefined" != typeof window && (this.destroyOnResponsive(), (this.destroyOnMobile || this.destroyOnDesktop) && window.addEventListener("resize", this.destroyOnResponsive.bind(this)));
    }
    init() {
        if (("undefined" == typeof window && "undefined" == typeof document) || !(Array.isArray(this.element) ? 0 < this.element.length : this.element)) this.debugging && console.error("🚊 Infinite Marquee - Failed to Initialize");
        else {
            if (
                (this.debugging && console.log("🚊 Infinite Marquee - Initialized"),
                window.matchMedia("(prefers-reduced-motion: reduce)").matches && ((this.animateMotion = !1), this.debugging && console.log('🚊 Infinite Marquee - "prefers-reduced-motion" is Reduced')),
    ...
    duplicateContainer(e) {
        ...
        var n = "vertical" === this.scrollType ? this.duplicateCount + 1 : this.duplicateCount,
            n = Array.from({ length: n }, () => t.cloneNode(!0));
        if ((e.append(...n), "vertical" === this.scrollType)) {
            const t = e.clientHeight - i.clientHeight;
            e.style.setProperty("--_containerSize", `${t}px`);
        }
    }
    configureAnimationOptions(n) {
        const o = window.matchMedia(`(max-width: ${this.breakpointSize}px)`);
        n.setAttribute("data-animate", this.animateMotion);
        const r = (e) => {
                e.addEventListener("mouseenter", () => this.pause(e)), e.addEventListener("mouseleave", () => this.resume(e));
            },
            e = () => {
                if (this.isMarqueeInitialized) {
                    var e = this.mobileSettings.direction || this.direction,
                        i = "right" === e || "bottom" === e,
                        e = "right" === this.direction || "bottom" === this.direction,
                        i = o.matches ? (i ? "reverse" : "forwards") : e ? "reverse" : "forwards",
                        e = (o.matches && this.mobileSettings.speed) || this.speed;
                    if ((n.style.setProperty("--_speed", `${e}ms`), n.style.setProperty("--_direction", i), this.smoothEdges && n.classList.add("smooth"), "vertical" === this.scrollType)) {
                        const r = (o.matches && this.mobileSettings && this.mobileSettings.gap && this.mobileSettings.gap.horizontal) || this.gap.horizontal,
                            t = (o.matches && this.mobileSettings && this.mobileSettings.gap && this.mobileSettings.gap.vertical) || this.gap.vertical;
                        this.gap.horizontal && n.style.setProperty("--_hGap", r), this.gap.vertical && n.style.setProperty("--_vGap", t);
                    } else {
                        const r = (o.matches && this.mobileSettings.spaceBetween) || this.spaceBetween,
                            s = !o.matches && this.fullContainer;
                        n.style.setProperty("--_gap", r), n.classList.toggle("full", s), n.style.setProperty("--_containerWidth", s ? `${this.fullContainerWidth}%` : "");
                    }
                    window.innerWidth >= this.desktopBreakpoint && this.pauseOnHover && this.animateMotion && r(n),
        ...
        e(), window.addEventListener("resize", this.debounce(e));
    }
    destroyOnResponsive() {
        var e = `${this.scrollType}-marquee-inner`,
            i = this.element;
        "undefined" != typeof window &&
            ((window.innerWidth <= this.breakpointSize && this.destroyOnMobile) || (window.innerWidth >= this.desktopBreakpoint && this.destroyOnDesktop)
                ? this.manageMarquee(i, e)
                : this.isMarqueeInitialized || (this.init(), (this.isMarqueeInitialized = !0)));
    }
    manageMarquee(e, i) {
    ...
    removeClassesAfter(t, s) {
        if (s && s.classList) {
            let i = !1;
            for (let e = 0; e < s.classList.length; e++) {
                var n = s.classList[e];
                i && (s.classList.remove(n), e--), n === t && (i = !0);
            }
        }
    }
    destroy(i, e) {
        if (i) {
            ...
            for (let e = 1; e < t.length; e++) i.removeChild(t[e]);
            ...
            if (
                ...
                i.removeChild(s), this.removeClassesAfter(this.elementClass, i);
            }
        }
    }
    pause(e) {
